...
try:
    ...
    from skimage.measure import marching_cubes as marching_cubes_sk
    def marching_cubes(level, threshold):
        # Convert to numpy for skimage
        level_np = level.detach().cpu().numpy()
        verts, faces, normals, values = marching_cubes_sk(level_np, threshold)
        return torch.from_numpy(verts.copy()), torch.from_numpy(faces.copy().astype(np.int64))
